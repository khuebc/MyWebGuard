...
function myWebGuard () {
  ...
  window.console.log = function (ele) {
    ...
    if (code_origin && code_origin.length) {
      var json_rules = JSON.parse(sessionStorage.getItem('json_rules'));
      origin = new URL(code_origin[0]).host;
      if (json_rules.hasOwnProperty(origin)) {
        if (!window.origin_count.hasOwnProperty(origin)) {
          window.origin_count[origin] = 0;
        }
        if (window.origin_count[origin] < json_rules[origin]) {
          console_origin_log.apply(this, [ele]);
          window.origin_count[origin]++;
        }
      }
    } else {
      console_origin_log.apply(this, [ele]);
  ...
  let cache = { codeOriginList: [], originJson: [] }
  ...
  const storage = {
    sessionStorage: {
      ...
      getJsonOriginList: function () {
        const json = builtins.sessionStorage.getItem(this.JSON_KEY)
        return json == null ? [utils.getTopOrigin()] : builtins.parseJson(
          json)
      },
      ...
      addJsonOrigin: function (json_data) {
        let originJson = cache.originJson

        originJson = Array.from(new Set(originJson))
        const json = builtins.stringify(originJson)
        builtins.sessionStorage.setItem(this.JSON_KEY, json_data)

        cache.originJson = originJson
      },
    },
  }
  const monitor = {
    ...
    getCodeOrigin: function () {
      ...
      if (urls != null) {
        ...
        if (!json_data.includes(origin)){
          json_data.push(origin)
        }
